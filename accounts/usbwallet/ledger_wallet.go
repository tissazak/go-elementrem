...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-ele/master/doc/eleapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Elementrem address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Elementrem transaction after having the user validate the parameters
...
type ledgerWallet struct {
	...
	version  [3]byte                                    // Current version of the Ledger Elementrem app (zero if app is offline)
	...
	deriveChain    elementrem.ChainStateReader // Blockchain state reader to discover used account with
...
// or whether the Elementrem app was not started on it.
func (w *ledgerWallet) Status() string {
	...
	if w.browser {
		return "Elementrem app in browser mode"
	}
	if w.offline() {
		return "Elementrem app offline"
	}
	return fmt.Sprintf("Elementrem app v%d.%d.%d online", w.version[0], w.version[1], w.version[2])
}

// offline returns whether the wallet and the Elementrem app is offline or not.
//
...
func (w *ledgerWallet) Open(passphrase string) error {
	...
	// Wallet seems to be successfully opened, guess if the Elementrem app is running
	...
	if _, err = w.ledgerDerive(accounts.DefaultBaseDerivationPath); err != nil {
		// Elementrem app is not running or in browser mode, nothing more to do, return
		if err == errReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Elementrem app's version, will fail prior to v1.0.2
...
func (w *ledgerWallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Elementrem account
			...
			// If the next account is empty, stop self-derivation, but add it noneleeless
...
func (w *ledgerWallet) SelfDerive(base accounts.DerivationPath, chain elementrem.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
//
// Note, if the version of the Elementrem application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested noneleeless, an error
...
func (w *ledgerWallet) SignTx(account accounts.Account, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	...
	// If the wallet is closed, or the Elementrem app doesn't run, abort
	if w.device == nil || w.offline() {
...
// ledgerVersion retrieves the current version of the Elementrem wallet app running
...
// ledgerDerive retrieves the currently active Elementrem address from a Ledger
...
//   Elementrem address length | 1 byte
//   Elementrem address        | 40 bytes hex ascii
...
func (w *ledgerWallet) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Elementrem hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Elementrem address and return
...
func (w *ledgerWallet) ledgerSign(derivationPath []uint32, address common.Address, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	...
	// Extract the Elementrem signature and do a sanity validation
	if len(reply) != 65 {
