...
var (
	// Files that end up in the gele*.zip archive.
	geleArchiveFiles = []string{
		"COPYING",
		executablePath("gele"),
	}

	// Files that end up in the gele-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			Name:        "gele",
			Description: "Elementrem CLI client.",
		},
		{
			Name:        "rlpdump",
		...
		{
			Name:        "evm",
			Description: "Developer utility version of the EVM (Elementrem Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			Name:        "swarm",
			Description: "Elementrem Swarm daemon and tools",
		},
		{
			Name:        "abigen",
			Description: "Source code generator to convert Elementrem contract definitions into easy to use, compile-time type-safe Go packages.",
		},
	}
...
func doArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archives (usually "gelestore/builds")`)
		ext    string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		...
		gele     = "gele-" + base + ext
		alltools = "gele-alltools-" + base + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gele, geleArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gele, alltools} {
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Where to upload the source package (usually "ppa:elementrem/elementrem")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gele-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debMetadata struct {
	...
	// go-elementrem version being built. Note that this
...
func newDebMetadata(distro, author string, env build.Environment, t time.Time) debMetadata {
	if author == "" {
		...
		author = "Elementrem <elementrem@gmail.com>"
	}
	return debMetadata{
		Env:         env,
		Author:      author,
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gele.exe" {
			geleTool = file
		} else {
	...
	// first section contains the gele binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Gele":     geleTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gele.nsi", filepath.Join(*workdir, "gele.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("gele-" + archiveBasename(*arch, env) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gele.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archive (usually "gelestore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	...
	if *local {
		...
		os.Rename("gele.aar", filepath.Join(GOBIN, "gele.aar"))
		return
	}
	...
	archive := "gele-" + archiveBasename("android", env) + ".aar"
	os.Rename("gele.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	archive := "gele-" + archiveBasename("ios", env)
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Gele.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Gele.podspec", "--allow-warnings", "--verbose")
	}
}
