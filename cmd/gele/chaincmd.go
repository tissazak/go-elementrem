...
Use "elementrem dump 0" to dump the genesis block.
`,
	}
)
...
func dbDirectory(db eledb.Database) string {
	ldb, ok := db.(*eledb.LDBDatabase)
	if !ok {
		return ""
	}
	return ldb.Path()
...
func closeAll(dbs ...eledb.Database) {
	for _, db := range dbs {
		db.Close()
	}
}
