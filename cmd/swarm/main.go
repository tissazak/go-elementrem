...
import (
	...
	"github.com/tissazak/go-elementrem/swarm"
	bzzapi "github.com/tissazak/go-elementrem/swarm/api"
...
var (
	...
	app              = utils.NewApp(gitCommit, "Elementrem Swarm")
...
var (
	...
	EleAPIFlag = cli.StringFlag{
		Name:  "eleapi",
		Usage: "URL of the Elementrem API provider",
		Value: node.DefaultIPCEndpoint("gele"),
	}
...
func init() {
	// Override flag defaults so bzzd can run alongside gele.
...
func registerBzzService(ctx *cli.Context, stack *node.Node) {
	...
	eleapi := ctx.GlobalString(EleAPIFlag.Name)
	cors := ctx.GlobalString(CorsStringFlag.Name)

	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		var client *eleclient.Client
		if len(eleapi) > 0 {
			client, err = eleclient.Dial(eleapi)
			if err != nil {
