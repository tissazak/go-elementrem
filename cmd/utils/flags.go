...
// Package utils contains internal helper functions for go-elementrem commands.
package utils
...
var (
	...
	ElementbaseFlag = cli.StringFlag{
		Name:  "elementbase",
	...
	EleStatsURLFlag = cli.StringFlag{
		Name:  "elestats",
		Usage: "Reporting URL of a elestats service (nodename:secret@host:port)",
	}
	...
	IPCPathFlag = DirectoryFlag{
		...
		Value: DirectoryString{"gele.ipc"},
	}
...
// MakeElementbase retrieves the elementbase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func MakeElementbase(ks *keystore.KeyStore, ctx *cli.Context) common.Address {
	...
	// If the specified elementbase is a valid address, return it
	account, err := MakeAddress(ks, elementbase)
	if err != nil {
		Fatalf("Option %q: %v", ElementbaseFlag.Name, err)
	}
...
func MakeNode(ctx *cli.Context, name, gitCommit string) *node.Node {
	...
	if ctx.GlobalBool(DevModeFlag.Name) {
		if !ctx.GlobalIsSet(DataDirFlag.Name) {
			config.DataDir = filepath.Join(os.TempDir(), "/elementrem_dev_mode")
		}
...
// RegisterEleService configures ele.Elementrem from command line flags and adds it to the
// given node.
func RegisterEleService(ctx *cli.Context, stack *node.Node, extra []byte) {
	...
	eleConf := &ele.Config{
		Elementbase:               MakeElementbase(ks, ctx),
	...
	case ctx.GlobalBool(TestNetFlag.Name):
		...
		eleConf.Genesis = core.DefaultTestnetGenesisBlock()

	case ctx.GlobalBool(DevModeFlag.Name):
		eleConf.Genesis = core.DevGenesisBlock()
		if !ctx.GlobalIsSet(GasPriceFlag.Name) {
			eleConf.GasPrice = new(big.Int)
		}
		eleConf.PowTest = true
	}
	// Override any global options pertaining to the Elementrem protocol
	if gen := ctx.GlobalInt(TrieCacheGenFlag.Name); gen > 0 {
		state.MaxTrieCacheGen = uint16(gen)
	}

	if eleConf.LightMode {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			return les.New(ctx, eleConf)
		}); err != nil {
			Fatalf("Failed to register the Elementrem light node service: %v", err)
		}
	} else {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			fullNode, err := ele.New(ctx, eleConf)
			if fullNode != nil && eleConf.LightServ > 0 {
				ls, _ := les.NewLesServer(fullNode, eleConf)
				fullNode.AddLesServer(ls)
			}
			return fullNode, err
		}); err != nil {
			Fatalf("Failed to register the Elementrem full node service: %v", err)
		}
	}
}
...
// RegisterEleStatsService configures the Elementrem Stats daemon and adds it to
// th egiven node.
func RegisterEleStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both ele and les services
		var eleServ *ele.Elementrem
		ctx.Service(&eleServ)

		var lesServ *les.LightElementrem
		ctx.Service(&lesServ)

		return elestats.New(url, eleServ, lesServ)
	}); err != nil {
		Fatalf("Failed to register the Elementrem Stats service: %v", err)
	}
}
...
func MakeChainConfigFromDb(ctx *cli.Context, db eledb.Database) *params.ChainConfig {
	...
	//  - If no genesis is set, we're running either mainnet or testnet (private nets use `gele init`)
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) eledb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb eledb.Database) {
	var err error
