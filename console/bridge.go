...
// RPC method (saved in jele.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call otto.FunctionCall) (response otto.Value) {
	...
	ret, err := call.Otto.Call("jele.newAccount", nil, password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
...
// original RPC method (saved in jele.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jele.unlockAccount", nil, account, passwd, duration)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
// jele.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call otto.FunctionCall) (response otto.Value) {
	...
	// Send the request to the backend and return
	val, err := call.Otto.Call("jele.sign", nil, message, account, passwd)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
func (b *bridge) SleepBlocks(call otto.FunctionCall) (response otto.Value) {
	...
	blockNumber := func() int64 {
		result, err := call.Otto.Run("ele.blockNumber")
		if err != nil {
			throwJSException(err.Error())
		}
