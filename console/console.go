...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jele", struct{}{})

	jeleObj, _ := c.jsre.Get("jele")
	jeleObj.Object().Set("send", bridge.Send)
	jeleObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jele);"); err != nil {
	...
	flatten := "var ele = web3.ele; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = ele.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// Assign these method in the Console the original web3 callbacks. These will be called by the jele.*
		// methods after they got the password from the user and send the original web3 request to the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jele.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jele.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jele.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines ele.getBalance(ele.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gele instance and some metadata about the
...
func (c *Console) Welcome() {
	...
	c.jsre.Run(`
		...
		console.log("at block: " + ele.blockNumber + " (" + new Date(1000 * ele.getBlock(ele.blockNumber).timestamp) + ")");
