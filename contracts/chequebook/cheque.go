...
// Package chequebook package wraps the 'chequebook' Elementrem smart contract.
//
// The functions in this package allow using chequebook for
// issuing, receiving, verifying cheques in element; (auto)cashing cheques in element
// as well as (auto)depositing element to the chequebook contract.
package chequebook
...
// * depositing element to the chequebook
// * watching incoming element
...
func (self *Chequebook) deposit(amount *big.Int) (string, error) {
	...
	if err != nil {
		glog.V(logger.Warn).Infof("error depositing %d mey to chequebook (%s, balance: %v, target: %v): %v", amount, self.contractAddr.Hex(), self.balance, self.buffer, err)
		return "", err
	}
	// assume that transaction is actually successful, we add the amount to balance right away
	self.balance.Add(self.balance, amount)
	glog.V(logger.Detail).Infof("deposited %d mey to chequebook (%s, balance: %v, target: %v)", amount, self.contractAddr.Hex(), self.balance, self.buffer)
...
func (self *Inbox) Receive(promise swap.Promise) (*big.Int, error) {
	...
	if err == nil {
		...
		glog.V(logger.Detail).Infof("received cheque of %v mey in inbox (%s, uncashed: %v)", amount, self.contract.Hex(), uncashed)
	}
...
// Cash cashes the cheque by sending an Elementrem transaction.
