...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	...
	var apiBackend eleapi.Backend
	var elementrem *ele.Elementrem
	if err := ctx.Service(&elementrem); err == nil {
		apiBackend = elementrem.ApiBackend
	} else {
		var elementrem *les.LightElementrem
		if err := ctx.Service(&elementrem); err == nil {
			apiBackend = elementrem.ApiBackend
		} else {
			return nil, err
	...
	contract, err := NewReleaseOracle(config.Oracle, ele.NewContractBackend(apiBackend))
	if err != nil {
		return nil, err
	}
