...
// Package state provides a caching layer atop the Elementrem state trie.
package state
...
// StateDBs within the elementrem protocol are used to store anything
...
func New(root common.Hash, db eledb.Database) (*StateDB, error) {
...
//
// Carrying over the balance ensures that Element doesn't disappear.
...
func (s *StateDB) CommitBatch(deleteEmptyObjects bool) (root common.Hash, batch eledb.Batch) {
	batch = s.db.NewBatch()
