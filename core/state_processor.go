...
// Process processes the state changes according to the Elementrem rules by running
...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, *big.Int, error) {
	...
	if p.config.INTERSTELLARleapSupport && p.config.INTERSTELLARleapBlock != nil && p.config.INTERSTELLARleapBlock.Cmp(block.Number()) == 0 {
