...
import (
	...
	"github.com/tissazak/go-elementrem/miner"
...
// PublicElementremAPI provides an API to access Elementrem full node-related
// information.
type PublicElementremAPI struct {
	e *Elementrem
}

// NewPublicElementremAPI creates a new Elementrem protocol API for full nodes.
func NewPublicElementremAPI(e *Elementrem) *PublicElementremAPI {
	return &PublicElementremAPI{e}
}

// Elementbase is the address that mining rewards will be send to
func (s *PublicElementremAPI) Elementbase() (common.Address, error) {
	return s.e.Elementbase()
}

// Coinbase is the address that mining rewards will be send to (alias for Elementbase)
func (s *PublicElementremAPI) Coinbase() (common.Address, error) {
	return s.Elementbase()
}

// Hashrate returns the POW hashrate
func (s *PublicElementremAPI) Hashrate() hexutil.Uint64 {
...
func NewPublicMinerAPI(e *Elementrem) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Elementrem) *PrivateMinerAPI {
...
// SetElementbase sets the elementbase of the miner
func (s *PrivateMinerAPI) SetElementbase(elementbase common.Address) bool {
	s.e.SetElementbase(elementbase)
	return true
}
...
func (s *PrivateMinerAPI) MakeDAG(blockNr rpc.BlockNumber) (bool, error) {
	if err := elhash.MakeDAG(uint64(blockNr.Int64()), ""); err != nil {
		return false, err
	}
	return true, nil
}

// PrivateAdminAPI is the collection of Elementrem full node-related APIs
...
// admin methods of the Elementrem service.
func NewPrivateAdminAPI(ele *Elementrem) *PrivateAdminAPI {
	return &PrivateAdminAPI{ele: ele}
}
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.ele.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.ele.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.ele.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of Elementrem full node APIs exposed
...
// related public debug methods of the Elementrem service.
func NewPublicDebugAPI(ele *Elementrem) *PublicDebugAPI {
	return &PublicDebugAPI{ele: ele}
}
...
func (api *PublicDebugAPI) DumpBlock(number uint64) (state.Dump, error) {
	...
	stateDb, err := api.ele.BlockChain().StateAt(block.Root())
	if err != nil {
...
// PrivateDebugAPI is the collection of Elementrem full node APIs exposed over
...
// private debug methods of the Elementrem service.
func NewPrivateDebugAPI(config *params.ChainConfig, ele *Elementrem) *PrivateDebugAPI {
	return &PrivateDebugAPI{config: config, ele: ele}
}
...
type BlockTraceResult struct {
	...
	StructLogs []eleapi.StructLogRes `json:"structLogs"`
...
func (api *PrivateDebugAPI) TraceBlock(blockRlp []byte, config *vm.LogConfig) BlockTraceResult {
	...
	return BlockTraceResult{
		Validated:  validated,
		StructLogs: eleapi.FormatLogs(logs),
...
func (api *PrivateDebugAPI) TraceBlockByHash(hash common.Hash, config *vm.LogConfig) BlockTraceResult {
	// Fetch the block that we aim to reprocess
	block := api.ele.BlockChain().GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, txHash common.Hash, config *TraceArgs) (interface{}, error) {
	...
	if config != nil && config.Tracer != nil {
		...
		var err error
		if tracer, err = eleapi.NewJavascriptTracer(*config.Tracer); err != nil {
			return nil, err
		}
		...
		go func() {
			<-deadlineCtx.Done()
			tracer.(*eleapi.JavascriptTracer).Stop(&timeoutError{})
		}()
		defer cancel()
	} else if config == nil {
	...
	tx, blockHash, _, txIndex := core.GetTransaction(api.ele.ChainDb(), txHash)
	if tx == nil {
		return nil, fmt.Errorf("transaction %x not found", txHash)
	}
	block := api.ele.BlockChain().GetBlockByHash(blockHash)
	if block == nil {
		return nil, fmt.Errorf("block %x not found", blockHash)
	}
	// Create the state database to mutate and eventually trace
	parent := api.ele.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		return nil, fmt.Errorf("block parent %x not found", block.ParentHash())
	}
	stateDb, err := api.ele.BlockChain().StateAt(parent.Root())
	if err != nil {
		return nil, err
	}
	...
	for idx, tx := range block.Transactions() {
		...
		context := core.NewEVMContext(msg, block.Header(), api.ele.BlockChain())
		...
		case *vm.StructLogger:
			return &eleapi.ExecutionResult{
				Gas:         gas,
				ReturnValue: fmt.Sprintf("%x", ret),
				StructLogs:  eleapi.FormatLogs(tracer.StructLogs()),
			}, nil
		case *eleapi.JavascriptTracer:
...
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	db := core.PreimageTable(api.ele.ChainDb())
