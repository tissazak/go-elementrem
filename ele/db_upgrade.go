...
// been safely stopped.
func upgradeSequentialKeys(db eledb.Database) (stopFn func()) {
...
func upgradeSequentialCanonicalNumbers(db eledb.Database, stopFn func() bool) (error, bool) {
	prefix := []byte("block-num-")
	it := db.(*eledb.LDBDatabase).NewIterator()
	defer func() {
		it.Release()
	}()
	it.Seek(prefix)
...
// if successful.
func upgradeSequentialBlocks(db eledb.Database, stopFn func() bool) (error, bool) {
	prefix := []byte("block-")
	it := db.(*eledb.LDBDatabase).NewIterator()
	defer func() {
		it.Release()
...
func upgradeSequentialOrphanedReceipts(db eledb.Database, stopFn func() bool) (error, bool) {
...
func upgradeSequentialBlockData(db eledb.Database, hash []byte) error {
