...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [ele/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	newPeerCh     chan *peer
	headerCh      chan dataPack        // [ele/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [ele/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [ele/63] Channel receiving inbound receipts
	stateCh       chan dataPack        // [ele/63] Channel receiving inbound node state data
	bodyWakeCh    chan bool            // [ele/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [ele/63] Channel to signal the receipt fetcher of new tasks
	stateWakeCh   chan bool            // [ele/63] Channel to signal the state fetcher of new tasks
	headerProcCh  chan []*types.Header // [ele/62] Channel to feed the header processor new tasks
...
func New(mode SyncMode, stateDb eledb.Database, mux *event.TypeMux, hasHeader headerCheckFn, hasBlockAndState blockAndStateCheckFn,
...
// these are zero.
func (d *Downloader) Progress() elementrem.SyncProgress {
