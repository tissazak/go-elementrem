...
// Block header and body fetchers belonging to ele/62 and above
...
type peer struct {
	...
	getAbsHeaders  absoluteHeaderFetcherFn // [ele/62] Method to retrieve a batch of headers from an absolute position
	getBlockBodies blockBodyFetcherFn      // [ele/62] Method to retrieve a batch of block bodies

	getReceipts receiptFetcherFn // [ele/63] Method to retrieve a batch of block transaction receipts
	getNodeData stateFetcherFn   // [ele/63] Method to retrieve a batch of state trie data

	version int // Ele protocol version number to switch strategies
	lock    sync.RWMutex
}
...
func (p *peer) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [ele/62+] requested on ele/%d", p.version))
	}
...
func (p *peer) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [ele/62+] requested on ele/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peer) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [ele/63+] requested on ele/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peer) FetchNodeData(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [ele/63+] requested on ele/%d", p.version))
	}
	// Short circuit if the peer is already fetching
