...
type fetchRequest struct {
	...
	From    uint64              // [ele/62] Requested chain element index (used for skeleton fills only)
	Hashes  map[common.Hash]int // [ele/61] Requested hashes with their insertion index (priority)
	Headers []*types.Header     // [ele/62] Requested headers, sorted by request order
...
type queue struct {
	...
	headerHead common.Hash // [ele/62] Hash of the last queued header to verify order

	// Headers are "special", they download in batches, supported by a skeleton chain
	headerTaskPool  map[uint64]*types.Header       // [ele/62] Pending header retrieval tasks, mapping starting indexes to skeleton headers
	headerTaskQueue *prque.Prque                   // [ele/62] Priority queue of the skeleton indexes to fetch the filling headers for
	headerPeerMiss  map[string]map[uint64]struct{} // [ele/62] Set of per-peer header batches known to be unavailable
	headerPendPool  map[string]*fetchRequest       // [ele/62] Currently pending header retrieval operations
	headerResults   []*types.Header                // [ele/62] Result cache accumulating the completed headers
	headerProced    int                            // [ele/62] Number of headers already processed from the results
	headerOffset    uint64                         // [ele/62] Number of the first header in the result cache
	headerContCh    chan bool                      // [ele/62] Channel to notify when header download finishes

	// All data retrievals below are based on an already assembles header chain
	blockTaskPool  map[common.Hash]*types.Header // [ele/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [ele/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [ele/62] Currently pending block (body) retrieval operations
	blockDonePool  map[common.Hash]struct{}      // [ele/62] Set of the completed block (body) fetches

	receiptTaskPool  map[common.Hash]*types.Header // [ele/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [ele/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [ele/63] Currently pending receipt retrieval operations
	receiptDonePool  map[common.Hash]struct{}      // [ele/63] Set of the completed receipt fetches

	stateTaskIndex int                      // [ele/63] Counter indexing the added hashes to ensure prioritised retrieval order
	stateTaskPool  map[common.Hash]int      // [ele/63] Pending node data retrieval tasks, mapping to their priority
	stateTaskQueue *prque.Prque             // [ele/63] Priority queue of the hashes to fetch the node data for
	statePendPool  map[string]*fetchRequest // [ele/63] Currently pending node data retrieval operations

	stateDatabase  eledb.Database   // [ele/63] Trie database to populate during state reassembly
	stateScheduler *state.StateSync // [ele/63] State trie synchronisation scheduler and integrator
	stateWriters   int              // [ele/63] Number of running state DB writer goroutines
...
func newQueue(stateDb eledb.Database) *queue {
	lock := new(sync.Mutex)
	return &queue{
