...
import (
	...
	"github.com/tissazak/go-elementrem/ele/fetcher"
...
func (pm *ProtocolManager) Stop() {
	glog.V(logger.Info).Infoln("Stopping elementrem protocol handler...")
	...
	glog.V(logger.Info).Infoln("Elementrem protocol handler stopped")
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= ele63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= ele63 && msg.Code == NodeDataMsg:
	...
	case p.version >= ele63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= ele63 && msg.Code == ReceiptsMsg:
