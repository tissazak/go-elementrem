...
// PeerInfo represents a short summary of the Elementrem sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Elementrem protocol version negotiated
...
// Handshake executes the ele protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("ele/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Elementrem sub-protocol.
type peerSet struct {
