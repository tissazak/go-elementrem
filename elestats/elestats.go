...
// Service implements an Elementrem netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	stack *node.Node // Temporary workaround, remove when API finalized

	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	ele    *ele.Elementrem      // Full Elementrem service if monitoring a full node
	les    *les.LightElementrem // Light Elementrem service if monitoring a light node
...
func New(url string, eleServ *ele.Elementrem, lesServ *les.LightElementrem) (*Service, error) {
...
func (s *Service) login(in *json.Decoder, out *json.Encoder) error {
	...
	if info := infos.Protocols["ele"]; info != nil {
		network = strconv.Itoa(info.(*ele.EleNodeInfo).Network)
		protocol = fmt.Sprintf("ele/%d", ele.ProtocolVersions[0])
	} else {
		network = strconv.Itoa(infos.Protocols["les"].(*ele.EleNodeInfo).Network)
...
func (s *Service) reportLatency(out *json.Encoder) error {
	// Send the current time to the elestats server
	start := time.Now()
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.ele != nil {
		...
		if block == nil {
			block = s.ele.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.ele.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(out *json.Encoder, list []uint64) error {
	...
	if len(list) > 0 {
		...
		if s.ele != nil {
			head = s.ele.BlockChain().CurrentHeader()
		} else {
	...
	for i, number := range indexes {
		if s.ele != nil {
			history[i] = s.assembleBlockStats(s.ele.BlockChain().GetBlockByNumber(number))
		} else {
...
func (s *Service) reportStats(out *json.Encoder) error {
	...
	if s.ele != nil {
		mining = s.ele.Miner().Mining()
		hashrate = int(s.ele.Miner().HashRate())

		sync := s.ele.Downloader().Progress()
		syncing = s.ele.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		gasprice = int(s.ele.Miner().GasPrice().Uint64())
	} else {
