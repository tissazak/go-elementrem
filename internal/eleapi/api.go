...
// PublicElementremAPI provides an API to access Elementrem related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicElementremAPI struct {
	b Backend
}

// NewPublicElementremAPI creates a new Elementrem protocol API.
func NewPublicElementremAPI(b Backend) *PublicElementremAPI {
	return &PublicElementremAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicElementremAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Elementrem protocol version this node supports
func (s *PublicElementremAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicElementremAPI) Syncing() (interface{}, error) {
...
func (s *PublicTxPoolAPI) Inspect() map[string]map[string]map[string]string {
	...
	var format = func(tx *types.Transaction) string {
		if to := tx.To(); to != nil {
			return fmt.Sprintf("%s: %v mey + %v × %v gas", tx.To().Hex(), tx.Value(), tx.Gas(), tx.GasPrice())
		}
		return fmt.Sprintf("contract creation: %v mey + %v × %v gas", tx.Value(), tx.Gas(), tx.GasPrice())
	}
	// Flatten the pending transactions
	for account, txs := range pending {
...
//   keccak256("\x19Elementrem Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Elementrem Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Elementrem ECDSA signature for:
// keccack256("\x19Elementrem Signed Message:\n" + len(message) + message))
//
...
// Note, this function is compatible with ele_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Elementrem Signed Message:\n"${message length}${message})
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Elementrem signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Elementrem blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Elementrem blockchain API.
...
// GetBalance returns the amount of mey for the given address in the state of the
...
func getTransaction(chainDb eledb.Database, b Backend, txHash common.Hash) (*types.Transaction, bool, error) {
...
func getTransactionBlockData(chainDb eledb.Database, txHash common.Hash) (common.Hash, uint64, uint64, error) {
	var txBlock struct {
...
// PublicDebugAPI is the collection of Elementrem APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	hash, err := elhash.GetSeedHash(number)
	if err != nil {
		return "", err
	}
...
// PrivateDebugAPI is the collection of Elementrem APIs exposed over the private
// debugging endpoint.
...
// Version returns the current elementrem protocol version.
