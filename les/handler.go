...
const (
	...
	eleVersion = 63 // equivalent ele version for the downloader
...
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, networkId int, mux *event.TypeMux, pow pow.PoW, blockchain BlockChain, txpool txPool, chainDb eledb.Database, odr *LesOdr, txrelay *LesTxRelay) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	// Unregister the peer from the downloader and Elementrem peer set
...
func (pm *ProtocolManager) Stop() {
	...
	glog.V(logger.Info).Infoln("Stopping light elementrem protocol handler...")
	...
	glog.V(logger.Info).Infoln("Light elementrem protocol handler stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if pm.lightSync {
		...
		if err := pm.downloader.RegisterPeer(p.id, eleVersion, p.HeadAndTd,
...
func (self *ProtocolManager) NodeInfo() *ele.EleNodeInfo {
	return &ele.EleNodeInfo{
