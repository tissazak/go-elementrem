...
import (
	...
	"github.com/tissazak/go-elementrem/les/flowcontrol"
...
func NewLesServer(ele *ele.Elementrem, config *ele.Config) (*LesServer, error) {
	pm, err := NewProtocolManager(config.ChainConfig, false, config.NetworkId, ele.EventMux(), ele.Pow(), ele.BlockChain(), ele.TxPool(), ele.ChainDb(), nil, nil)
	if err != nil {
		return nil, err
	}
	pm.blockLoop()
	...
	srv.fcCostStats = newCostStats(ele.ChainDb())
	return srv, nil
}
...
func newCostStats(db eledb.Database) *requestCostStats {
...
func getChtRoot(db eledb.Database, num uint64) common.Hash {
...
func storeChtRoot(db eledb.Database, num uint64, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], num)
	db.Put(append(chtPrefix, encNumber[:]...), root[:])
}

func makeCht(db eledb.Database) bool {
