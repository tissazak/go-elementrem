...
func newWorker(config *params.ChainConfig, coinbase common.Address, ele Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:         config,
		ele:            ele,
		mux:            mux,
		...
		chain:          ele.BlockChain(),
		proc:           ele.BlockChain().Validator(),
		possibleUncles: make(map[common.Hash]*types.Block),
		coinbase:       coinbase,
		txQueue:        make(map[common.Hash]*types.Transaction),
		agents:         make(map[Agent]struct{}),
		unconfirmed:    newUnconfirmedBlocks(ele.BlockChain(), 5),
...
func (self *worker) setElementbase(addr common.Address) {
	self.mu.Lock()
	...
	for {
		...
		for result := range self.recv {
			...
			if self.fullValidation {
				...
				auxValidator := self.ele.BlockChain().AuxValidator()
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	wallets := self.ele.AccountManager().Wallets()
...
func (self *worker) commitNewWork() {
	...
	if interstellarBlock := self.config.INTERSTELLARleapBlock; interstellarBlock != nil {
		...
		if header.Number.Cmp(interstellarBlock) >= 0 && header.Number.Cmp(limit) < 0 {
			...
			} else if bytes.Equal(header.Extra, params.INTERSTELLARleapBlockExtra) {
	...
	pending, err := self.ele.TxPool().Pending()
	if err != nil {
	...
	self.ele.TxPool().RemoveBatch(work.lowGasTxs)
	self.ele.TxPool().RemoveBatch(work.failedTxs)
