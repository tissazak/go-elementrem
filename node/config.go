...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gele is "gele". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gele", keep that.
	if name == "gele" || name == "gele-testnet" {
		name = "Gele"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gele" instances.
var isOldGeleResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by gele 1.4 are used if they exist.
	if c.name() == "gele" && isOldGeleResource[path] {
		oldpath := ""
		if c.Name == "gele" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	default:
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-elementrem-keystore")
		ephemeral = keydir
	}
	if err != nil {
		return nil, "", err
