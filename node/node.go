...
func (n *Node) OpenDatabase(name string, cache, handles int) (eledb.Database, error) {
	if n.config.DataDir == "" {
		return eledb.NewMemDatabase()
	}
	return eledb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
