...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (eledb.Database, error) {
	if ctx.config.DataDir == "" {
		return eledb.NewMemDatabase()
	}
	return eledb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
}
