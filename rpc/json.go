...
const (
	...
	subscribeMethod        = "ele_subscribe"
	unsubscribeMethod      = "ele_unsubscribe"
	notificationMethod     = "ele_subscription"
)
...
func parseRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	...
	if in.Method == subscribeMethod {
		...
		if len(in.Payload) > 0 {
			...
			// all subscriptions are made on the ele service
			reqs[0].service, reqs[0].method = "ele", subscribeMethod[0]
	...
	for i, r := range in {
		...
		if r.Method == subscribeMethod {
			...
			if len(r.Payload) > 0 {
				...
				// all subscriptions are made on the ele service
				requests[i].service, requests[i].method = "ele", subscribeMethod[0]
