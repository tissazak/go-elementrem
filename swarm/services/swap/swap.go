...
var (
	...
	autoCashThreshold    = big.NewInt(50000000000000)  // threshold that triggers autocash (mey)
	autoDepositInterval  = 300 * time.Second           // default interval for autocash
	autoDepositThreshold = big.NewInt(50000000000000)  // threshold that triggers autodeposit (mey)
	autoDepositBuffer    = big.NewInt(100000000000000) // buffer that is surplus for fork protection etc (mey)
	buyAt                = big.NewInt(20000000000)     // maximum chunk price host is willing to pay (mey)
	sellAt               = big.NewInt(20000000000)     // minimum chunk price host requires (mey)
...
func NewSwap(local *SwapParams, remote *SwapProfile, backend chequebook.Backend, proto swap.Protocol) (self *swap.Swap, err error) {
	...
	var buy, sell string
	if self.Buys {
		buy = "purchase from peer enabled at " + remote.SellAt.String() + " mey/chunk"
	} else {
		buy = "purchase from peer disabled"
	}
	if self.Sells {
		sell = "selling to peer enabled at " + local.SellAt.String() + " mey/chunk"
	} else {
