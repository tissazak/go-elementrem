...
/*
#include "src/libelhash/internal.h"

int elhashGoCallback_cgo(unsigned);
*/
import "C"
...
func defaultDir() string {
	...
	if runtime.GOOS == "windows" {
		return filepath.Join(home, "AppData", "Elhash")
	}
	return filepath.Join(home, ".elhash")
}

// cache wraps an elhash_light_t with some metadata
...
func (cache *cache) generate() {
	cache.gen.Do(func() {
		...
		size := C.elhash_get_cachesize(C.uint64_t(cache.epoch * epochLength))
		if cache.test {
			size = cacheSizeForTesting
		}
		cache.ptr = C.elhash_light_new_internal(size, (*C.elhash_h256_t)(unsafe.Pointer(&seedHash[0])))
...
func freeCache(cache *cache) {
	C.elhash_light_delete(cache.ptr)
	cache.ptr = nil
}

func (cache *cache) compute(dagSize uint64, hash common.Hash, nonce uint64) (ok bool, mixDigest, result common.Hash) {
	ret := C.elhash_light_compute_internal(cache.ptr, C.uint64_t(dagSize), hashToH256(hash), C.uint64_t(nonce))
...
func (l *Light) Verify(block pow.Block) bool {
	// TODO: do elhash_quick_verify before getCache in order
	...
	/* Cannot happen if block header diff is validated prior to PoW, but can
	   ...
	   Elementrem protocol consensus rules here which are not in scope of Elhash
	*/
	...
	dagSize := C.elhash_get_datasize(C.uint64_t(blockNum))
	if l.test {
...
func h256ToHash(in C.elhash_h256_t) common.Hash {
	return *(*common.Hash)(unsafe.Pointer(&in.b))
}

func hashToH256(in common.Hash) C.elhash_h256_t {
	return C.elhash_h256_t{b: *(*[32]C.uint8_t)(unsafe.Pointer(&in[0]))}
}
...
// dag wraps an elhash_full_t with some metadata
// and automatic memory management.
type dag struct {
	epoch uint64
	test  bool
...
func (d *dag) generate() {
	d.gen.Do(func() {
		var (
			...
			cacheSize = C.elhash_get_cachesize(blockNum)
			dagSize   = C.elhash_get_datasize(blockNum)
		)
		if d.test {
		...
		cache := C.elhash_light_new_internal(cacheSize, (*C.elhash_h256_t)(unsafe.Pointer(&seedHash[0])))
		defer C.elhash_light_delete(cache)
		// Generate the actual DAG.
		d.ptr = C.elhash_full_new_internal(
			C.CString(d.dir),
			hashToH256(seedHash),
			dagSize,
			cache,
			(C.elhash_callback_t)(unsafe.Pointer(C.elhashGoCallback_cgo)),
		)
		if d.ptr == nil {
			panic("elhash_full_new IO or memory error")
		}
...
//export elhashGoCallback
func elhashGoCallback(percent C.unsigned) C.int {
...
func (pow *Full) Search(block pow.Block, stop <-chan struct{}, index int) (nonce uint64, mixDigest []byte) {
	...
	for {
		...
		default:
			...
			ret := C.elhash_full_compute(dag.ptr, hash, C.uint64_t(nonce))
			result := h256ToHash(ret.result).Big()

			// TODO: disagrees with the spec https://github.com/elementrem
...
// Elhash combines block verification with Light and
...
func New() *Elhash {
	return &Elhash{new(Light), &Full{turbo: true}}
}
...
func NewShared() *Elhash {
	return &Elhash{sharedLight, &Full{turbo: true}}
}
...
// regular-size cache.
func NewForTesting() (*Elhash, error) {
	dir, err := ioutil.TempDir("", "elhash-test")
	if err != nil {
		return nil, err
	}
	return &Elhash{&Light{test: true}, &Full{Dir: dir, test: true}}, nil
}
