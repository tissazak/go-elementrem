...
import (
	...
	"github.com/tissazak/go-elementrem/whisper"
)
...
func main() {
	...
	// Create an Elementrem peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting Elementrem peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start Elementrem peer: %v.\n", err)
		os.Exit(1)
	}
